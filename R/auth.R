...
#' @keywords internal
.gd_drive_auth <- function(email) googledrive::drive_auth(email = email)
...
#' from the `GDRIVE_EMAIL` environment variable. The token is cached and reused
#' in subsequent calls.
#'
...
gdpins_ensure_drive_auth <- function(email = Sys.getenv("GDRIVE_EMAIL")) {
  if (nchar(email) == 0L) {
    cli::cli_abort(c(
      "Google Drive email is not set.",
      x = "{.envvar GDRIVE_EMAIL} is empty.",
      i = paste0(
        "Add {.code GDRIVE_EMAIL=you@example.com} to your ",
        "{.file ~/.Renviron} and restart R, or pass {.arg email} directly."
      )
    ))
  }
