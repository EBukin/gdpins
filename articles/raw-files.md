...
You do not have to remember the exact path.
[`gdpins_raw_path()`](https://ebukin.github.io/gdpins/reference/gdpins_raw_path.md),
[`gdpins_raw_get()`](https://ebukin.github.io/gdpins/reference/gdpins_raw_get.md)
and
[`gdpins_pin_read()`](https://ebukin.github.io/gdpins/reference/gdpins_pin_read.md)
resolve the name you pass against what is actually there:

``` r

# The file lives at "api/worldbank/gdp_2024.parquet" -- the basename is enough,
# as long as it is unique.
gdpins_raw_get(conn, "gdp_2024.parquet")

# Case does not have to match either.
gdpins_raw_get(conn, "GDP_2024.parquet")

# A near-miss is never guessed at -- you get told, with candidates:
gdpins_raw_get(conn, "gdp_2024.csv")
#> Error: File not found in raw connection: 'gdp_2024.csv'
#> i Did you mean:
#> * api/worldbank/gdp_2024.parquet
#> i List everything with `gdpins_raw_get(conn, "*")`.
```

The rule is that gdpins auto-resolves **only** when the match is both
exact and unique. Anything looser is reported rather than acted on, so
it can never quietly read a different file than you meant. Two basenames
that clash are an error listing both full paths, not a coin flip.

[`gdpins_raw_remove()`](https://ebukin.github.io/gdpins/reference/gdpins_raw_remove.md)
opts out entirely: it matches the exact path only. It deletes your local
copy, so it is the one verb that must never guess.

## Listing mode: `"*"`

A name containing `*` or `?` flips these verbs into **listing mode** —
they show you what matches instead of acting on it. Nothing is read;
nothing is deleted.

``` r

# Everything, at any depth
gdpins_raw_path(conn, "*")
#> 4 files
#> * gdp_2024.parquet
#> * metadata.csv
#> * api/worldbank/gdp_2024.parquet
#> * sub/sub/folder/notes.rds

# Every CSV, however deeply nested
gdpins_raw_path(conn, "*.csv")
```

This is the quickest way to answer “what is actually in here?”, and it
is what the error messages above point you at. Unlike
[`gdpins_raw_ls()`](https://ebukin.github.io/gdpins/reference/gdpins_raw_ls.md)
— whose `depth = 2` default hides `sub/sub/folder/notes.rds` — listing
mode always recurses the whole tree. Matching is case-sensitive on every
platform, so `"*.csv"` will not match `CARS.CSV`.

The result is an ordinary tibble (it just prints as names), so you can
go straight on with it:

``` r

big <- gdpins_raw_path(conn, "*.parquet")
big[big$size > 1e6, c("name", "size")]
```
