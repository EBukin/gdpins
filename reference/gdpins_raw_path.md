...
naming those formats and pointing at `gdpins_raw_path()`.

`gdpins_raw_path()` returns a path for **any** extension, downloading
from Drive on demand. It is the escape hatch for formats gdpins does not
read: get the path, then hand it to whatever package does.
...
#> [1] "/tmp/RtmplHQDVx/raw_1e0c57ff1156/quarterly report (Q1 2024).csv"
