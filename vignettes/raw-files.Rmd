...
You do not have to remember the exact path. `gdpins_raw_path()`,
`gdpins_raw_get()` and `gdpins_pin_read()` resolve the name you pass against
...
The rule is that gdpins auto-resolves **only** when the match is both exact and
unique. Anything looser is reported rather than acted on, so it can never
quietly read a different file than you meant. Two basenames that clash are an
error listing both full paths, not a coin flip.

`gdpins_raw_remove()` opts out entirely: it matches the exact path only. It
deletes your local copy, so it is the one verb that must never guess.

## Listing mode: `"*"`

A name containing `*` or `?` flips these verbs into **listing mode** — they show
you what matches instead of acting on it. Nothing is read; nothing is deleted.
...
This is the quickest way to answer "what is actually in here?", and it is what
the error messages above point you at. Unlike `gdpins_raw_ls()` — whose
`depth = 2` default hides `sub/sub/folder/notes.rds` — listing mode always
recurses the whole tree. Matching is case-sensitive on every platform, so
`"*.csv"` will not match `CARS.CSV`.

The result is an ordinary tibble (it just prints as names), so you can go
straight on with it:
